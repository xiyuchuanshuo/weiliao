...
import com.b509.xiaoliao.MainActivity;
import com.b509.xiaoliao.MyApplication;
import com.b509.xiaoliao.inter.OnUDPReceiveMessage;
...
//	    Log.d("====", "收到消息："+msg.toString());
	    ...
	    switch (type) {
				case ADD_USER://增加一个用户
					...
					//构造回送报文内容
					if(!MyApplication.mainInstance.getLocalIp().equals(user.getIp())){
						users.put(sourceIp, user);
						send(MyApplication.mainInstance.getMyUdpMessage("", LOGIN_SUCC).toString(), packet.getAddress());
					}
					break;
				...
				case HEART_BEAT://心跳包检测
					send(MyApplication.mainInstance.getMyUdpMessage("", HEART_BEAT_REPLY).toString(), packet.getAddress());//回复心跳包
				...
				case REQUIRE_ICON://请求头像
					File file=new File(MyApplication.iconPath+MainActivity.iconName);
					if(file.exists()){
						...
						listener.sendFile(sourceIp, file, MyApplication.iconPath,MyApplication.mainInstance.getDeviceCode());
					}
					break;
			}
  ...
  void noticeOffline() {
		try {
			send(MyApplication.mainInstance.getMyUdpMessage("", REMOVE_USER).toString(), InetAddress.getByName(Constant.ALL_ADDRESS));
	...
	public void noticeOnline(){
		try {
			send(MyApplication.mainInstance.getMyUdpMessage("", ADD_USER).toString(), InetAddress.getByName(Constant.ALL_ADDRESS));
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
	}
